...
const GLchar * linked_render_source [] = {
R"(#version 430
...
void main()
{
	...
	//diffuse = vec4(frag_count/16.0,0,0,1);
...
oit_linked::oit_linked(int w, int h, int s) {
   ...
   // COMPUTE SHADERS
    ...
    glShaderSource(render_shader_part, 1, linked_render_source, NULL);
	...
	gl4::Shader *oit_render = new gl4::Shader("../shaders/oit_linked_render_VS.glsl", "../shaders/oit_linked_render_FS.glsl");
	gl4::ShaderManager::getInstance()->addShaderProgram("oit_linked_render", oit_render);

}

oit_linked::~oit_linked() {

}

void oit_linked::render(const GLfloat *camera_transform, GLuint out_texture, void (*f)(void)) {
	glBindBuffer(GL_PIXEL_UNPACK_BUFFER, head_pointer_initializer);
	glBindTexture(GL_TEXTURE_2D, head_pointer_texture);
	...
	gl4::ShaderManager::getInstance()->bindShader("oit_linked_render");

	// Bind head-pointer image for read-write
    glBindImageTexture(0, head_pointer_texture, 0, GL_FALSE, 0, GL_READ_WRITE, GL_R32UI);
    glBindImageTexture(1, linked_list_texture, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA32UI);
	glBindImageTexture(2, out_texture, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA32F);
