...
const GLchar * dynamic_render_source [] = {
R"(#version 430
...
oit_dynamic::oit_dynamic(int w, int h, int s) {
   ...
   // COMPUTE SHADERS
    ...
    glShaderSource(render_shader_part, 1, dynamic_render_source, NULL);
	...
	gl4::Shader *oit_render = new gl4::Shader("../shaders/oit_dynamic_render_VS.glsl", "../shaders/oit_dynamic_render_FS.glsl");
	gl4::ShaderManager::getInstance()->addShaderProgram("oit_dynamic_render", oit_render);
...
oit_dynamic::~oit_dynamic() {

}

void oit_dynamic::render(const GLfloat *camera_transform, GLuint out_texture, void (*f)(void)) {
	...
	glBindTexture(GL_TEXTURE_2D, head_pointer_texture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_R32UI, max_width, max_height, 0, GL_RED_INTEGER, GL_UNSIGNED_INT, get_pixel_buffer);
	...
	gl4::ShaderManager::getInstance()->bindShader("oit_dynamic_render");
